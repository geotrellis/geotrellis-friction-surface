...
object CougToblerLogger {
  def getLogger = Logger.getLogger(CougTobler.getClass)
}


object CougTobler extends CommandApp(
  ...
  main   = {
    ...
    (partO, pathO, outpO, tinyF).mapN { (numPartitions, orcPath, resultName, tiny) =>
      ...
      val logger = CougToblerLogger.getLogger
      ...
      try {
        ...
        pyramid.foreach { case (z, rdd) => logger.debug(s"\n ZOOM: $z COUNT: ${rdd.count}") }
